...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const switchManager = {
        profiles: [
            { tag: 'switch_ebpf', name: 'eBPF 内核加速', desc: '启用 eBPF Sockmap 旁路加速 (Performance++)', tip: '需 Root 权限。极速转发 MosDNS 于 Sing-box 之间的流量。', valueForOn: 'true' },
        ...
        async loadStatus(signal) {
            try {
                // Separate eBPF from plugin switches
                const pluginProfiles = this.profiles.filter(p => p.tag !== 'switch_ebpf');
                const fetchPromises = pluginProfiles.map(p => api.fetch(`/plugins/${p.tag}/show`, { signal }));

                // Add eBPF status check
                const ebpfProfile = this.profiles.find(p => p.tag === 'switch_ebpf');
                let ebpfPromise = null;
                if (ebpfProfile) {
                    ebpfPromise = api.ebpf.getStatus(signal).catch(e => ({ active: false, error: e.message || 'Network Error' }));
                }

                const results = await Promise.allSettled(fetchPromises);

                results.forEach((result, index) => {
                    const profile = pluginProfiles[index];
                ...
                // Handle eBPF result
                if (ebpfPromise) {
                    const ebpfRes = await ebpfPromise;

                    if (ebpfRes.error) {
                        console.warn("eBPF Status Error:", ebpfRes.error);
                        // Optional: show toast only if it's a backend error, not just 'inactive'
                        // But verifying backend execution error is useful here.
                        if (ebpfRes.error !== 'Network Error') {
                            ui.showToast(`eBPF 检查警告: ${ebpfRes.error}`, 'warning');
                        }
                    }

                    state.featureSwitches['switch_ebpf'] = ebpfRes.active ? 'true' : 'false';
                    state.ebpfStatus = ebpfRes;

                    // Optional: update tip with PID if active
                    if (ebpfRes.active && ebpfRes.pid) {
                        const p = this.profiles.find(x => x.tag === 'switch_ebpf');
                        if (p) p.tip = `运行中 (PID: ${ebpfRes.pid})`;
                    }

                    // Refresh System Info panel to show eBPF status
                    if (systemInfoManager && typeof systemInfoManager.update === 'function') {
                        systemInfoManager.update();
                    }
                }
        ...
        async handleSecondarySwitch(checkbox) {
            ...
            // --- Special Handling for eBPF ---
            if (tag === 'switch_ebpf') {
                const enable = checkbox.checked;
                try {
                    await api.ebpf.toggle(enable);
                    state.featureSwitches[tag] = enable ? 'true' : 'false';
                    ui.showToast(`eBPF 加速已${enable ? '启用' : '禁用'}`, 'success');
                } catch (error) {
                    ui.showToast(`切换 eBPF 状态失败: ${error.message}`, 'error');

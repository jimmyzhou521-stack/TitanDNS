...
use crate::plugins::forward::{ForwardPlugin, Upstream};
...
pub struct SmartForwardPlugin {
    ...
    pub persist_data: Arc<tokio::sync::RwLock<HashMap<String, bool>>>,
}
...
impl SmartForwardPlugin {
    pub async fn new(
        ...
        let local_target = if !local_upstreams.is_empty() {
            ...
            let mut fp = ForwardPlugin::new(
                format!("{}::local", name),
                local_upstreams,
                Some(strategy),
                local_concurrent.unwrap_or(2),
            );
            if let Some(ms) = local_timeout_ms {
                fp.timeout = Duration::from_millis(ms);
            }
            ...
            let upstream = Upstream::new(conf).context("Failed to create local upstream")?;
        ...
        let fake_target = if !fakeip_upstreams.is_empty() {
            ...
            if let Some(ms) = fakeip_timeout_ms {
                fp.timeout = Duration::from_millis(ms);
            }
            ...
            let upstream = Upstream::new(conf).context("Failed to create fakeip upstream")?;
            let label = upstream.get_label();
            crate::autopilot::register_upstream(&label);
            SmartForwardTarget::Single {
                upstream: Arc::new(upstream),
                label,
                timeout: fakeip_timeout_ms
                    .map(Duration::from_millis)
                    .unwrap_or(default_timeout),
            }
        };
        ...
        let fallback_target = if !local_fallback_upstreams.is_empty() {
            ...
            if let Some(ms) = local_fallback_timeout_ms {
                fp.timeout = Duration::from_millis(ms);
            }
        ...
        // Cache 10,000 domains for 1 hour
        ...
        let persist_data: HashMap<String, bool> = HashMap::new();
        let persist_data = Arc::new(tokio::sync::RwLock::new(persist_data));
            ...
            if path.exists() {
                match Self::load_learning_cache_from_file(path) {
                    Ok(data) => {
                        ...
                        {
                            let mut pd = persist_data.write().await;
                            for (domain, is_cn) in data {
                                cache.insert(domain.clone(), is_cn).await;
                                pd.insert(domain, is_cn);
                            }
                        }
                        info!("📂 SmartForward '{}': Loaded {} learning entries from {:?}", name, count, path);
                    }
                    Err(e) => {
                        warn!("⚠️ SmartForward '{}': Failed to load learning cache: {}", name, e);
                    }
                }
            } else {
                info!("📂 SmartForward '{}': Persistence file {:?} not found, starting fresh", name, path);
            }
        }
        ...
        Ok(Self {
            ...
            domestic_suffixes: domestic_suffixes
                .into_iter()
    ...
    pub async fn save_learning_cache(&self) -> Result<()> {
        if let Some(ref path) = self.persist_file {
            ...
            let data = self.persist_data.read().await;
            let count = data.len();
            ...
            info!("📦 SmartForward '{}': Saved {} learning entries to {:?}", self.name, count, path);
        }
        Ok(())
    }

    /// Record a learning result (updates both cache and persist_data)
    pub async fn record_learning(&self, domain: String, is_cn: bool) {
        self.learning_cache.insert(domain.clone(), is_cn).await;
        self.persist_data.write().await.insert(domain, is_cn);
    }
    ...
    pub fn start_persistence_task(self: Arc<Self>) {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                let current_count = plugin.persist_data.read().await.len();
                if current_count > last_count {
                    if let Err(e) = plugin.save_learning_cache().await {
                        warn!("⚠️ SmartForward '{}': Persistence task failed: {}", plugin.name, e);
                    }
        ...
        info!("🔄 SmartForward '{}': Started background persistence task (every 5 min)", self.name);
    }

    async fn exchange_upstream(&self, upstream: &Upstream, req: &Message, timeout: Duration) -> Result<Message> {
        match tokio::time::timeout(timeout, upstream.exchange(req)).await {
            Ok(Ok(resp)) => Ok(resp),
            Ok(Err(e)) => Err(e),
    ...
    async fn exchange_target(&self, target: &SmartForwardTarget, req: &Message, client_ip: IpAddr) -> Result<Message> {
        match target {
            SmartForwardTarget::Single { upstream, timeout, .. } => {
                self.exchange_upstream(upstream, req, *timeout).await
            }
            SmartForwardTarget::Group { forward, .. } => {
                forward.execute(req, Some(client_ip)).await
            }
        }
    }
    ...
    async fn classify_response(&self, resp: &Message) -> (bool, bool) {
        ...
        if !has_ip {
            for ans in resp.additionals() {
                ...
                if let Some(ip) = ip {
                    has_ip = true;
                    debug!("🔍 Found IP in ADDITIONAL section: {} (from CNAME target)", ip);
    ...
    fn is_domestic_domain(&self, qname: &str) -> bool {
        if self.domestic_suffixes.is_empty() {
            return qname.ends_with(".cn") || qname.ends_with(".cn.");
        }
        let name = qname.to_lowercase();
        self.domestic_suffixes.iter().any(|s| name.ends_with(s))
    }
}
...
impl Plugin for SmartForwardPlugin {
    ...
    async fn handle(&self, ctx: &mut Context) -> Result<()> {
        ...
        let qname = if let Some(q) = request.query() {
            q.name().to_string()
        } else {
            return Ok(());
        };
        ...
        crate::autopilot::record_domain_query(&qname);
        crate::autopilot::record_timed_access(&qname);

        // --- Step -1: AI 域名记忆快速通道 (基于 AutoPilot) ---
        let local_label = self.local.label().to_string();
        let fake_label = self.fakeip.label().to_string();
        if let Some(best_label) = crate::autopilot::get_domain_best_upstream_for_client(&qname, client_ip)
            .or_else(|| crate::autopilot::get_domain_best_upstream(&qname))
        {
            if self.local.matches_label(&best_label) {
                debug!("🤖 AI记忆命中: {} → Local", qname);
                let start = Instant::now();
                match self.exchange_target(&self.local, request, client_ip).await {
                    Ok(mut resp) => {
                        let elapsed = start.elapsed().as_millis() as u64;
                        crate::autopilot::record_success(&local_label, elapsed);
                        crate::autopilot::record_domain_result_for_client(&qname, &local_label, elapsed, client_ip);
                        self.record_learning(qname.clone(), true).await;
                        resp.set_id(id);
                ...
                if self.avoid_fakeip_on_domestic && self.is_domestic_domain(&qname) {
                    debug!("🛡️ AI记忆被纠正: {} 匹配国内后缀，强制走 Local (避免 FakeIP)", qname);
                    let start = Instant::now();
                    match self.exchange_target(&self.local, request, client_ip).await {
                        Ok(mut resp) => {
                            let elapsed = start.elapsed().as_millis() as u64;
                    ...
                    let start = Instant::now();
                    match self.exchange_target(&self.fakeip, request, client_ip).await {
                        Ok(mut resp) => {
                            let elapsed = start.elapsed().as_millis() as u64;
                            crate::autopilot::record_success(&fake_label, elapsed);
                            crate::autopilot::record_domain_result_for_client(&qname, &fake_label, elapsed, client_ip);
                            self.record_learning(qname.clone(), false).await;
                            resp.set_id(id);
                            ctx.set_response(resp, true);
                            return Ok(());
                        }
        ...
        // --- Step 0: Check Auto-Learning Cache ---
        if let Some(is_cn) = self.learning_cache.get(&qname).await {
            if is_cn {
                debug!("🧠 SmartForward (Learned): {} is CN. Routing to Local.", qname);
                ...
                if self.avoid_fakeip_on_domestic && self.is_domestic_domain(&qname) {
                    debug!("🛡️ 学习缓存被纠正: {} 匹配国内后缀，强制走 Local (避免 FakeIP)", qname);
                    ...
                    debug!("🧠 SmartForward (Learned): {} is Global. Routing to FakeIP.", qname);
        ...
        debug!("🕵️ SmartForward: Probing with Local Upstream for {}...", qname);
        let probe_start = Instant::now();
        let local_res = self.exchange_target(&self.local, request, client_ip).await;
        ...
        match local_res {
            Ok(mut local_resp) => {
                ...
                if is_cn {
                    ...
                    self.record_learning(qname.clone(), true).await;
                    crate::autopilot::record_domain_result_for_client(&qname, &local_label, probe_elapsed, client_ip);
                    ...
                    // 这样可以避免影响国外域名的正常流程
                    if self.is_domestic_domain(&qname) {
                        local_resp_for_fallback = Some(local_resp.clone());
                    }
                }
            },
            Err(e) => {
                crate::autopilot::record_failure(&local_label);
                warn!("⚠️ SmartForward: Local probe failed ({}). Trying fallback...", e);

                if let Some(fallback) = &self.local_fallback {
                    let fb_label = fallback.label().to_string();
                    let fb_start = Instant::now();
                    match self.exchange_target(fallback, request, client_ip).await {
                        Ok(mut fb_resp) => {
                            ...
                            if is_cn {
                                ...
                                crate::autopilot::record_domain_result_for_client(&qname, &fb_label, fb_elapsed, client_ip);
                                fb_resp.set_id(id);
                                ...
                                debug!("🧯 SmartForward: Fallback indicates Foreign. Continue FakeIP.");
                                self.record_learning(qname.clone(), false).await;
                                // 🔧 [Bug #6 Fix - 保守版本] 只对国内后缀域名保存响应
                                if self.is_domestic_domain(&qname) {
                                    local_resp_for_fallback = Some(fb_resp.clone());
                        ...
                        Err(err) => {
                            ...
                            warn!("⚠️ SmartForward: Fallback failed ({}). Continue FakeIP.", err);
                        }
                    }
                } else {
        ...
        if self.avoid_fakeip_on_domestic && self.is_domestic_domain(&qname) {
            ...
            if let Some(mut local_resp) = local_resp_for_fallback {
                ...
                warn!("🚫 SmartForward: Domestic domain '{}' and local failed, returning SERVFAIL", qname);
        ...
        let fake_res = self.exchange_target(&self.fakeip, request, client_ip).await;
        ...
        match fake_res {
            Ok(mut fake_resp) => {
                ...
                crate::autopilot::record_domain_result_for_client(&qname, &fake_label, fake_elapsed, client_ip);
                fake_resp.set_id(id);
